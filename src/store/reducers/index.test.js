...
describe('game', () => {
  ...
  it('should update a co-ordinate to match the currentPlayer', () => {
    const xState = { currentPlayer: 'X', winner: null, end: false };
    const oState = { currentPlayer: 'O', winner: null, end: false };
  ...
  it('should update winner when game is won', () => {
    const expectedState = {
      winner: 'X',
      end: true,
      currentPlayer: 'X',
    ...
    const result = game(xState, Actions.declareWinner('X'));

    expect(result).toEqual(expectedState);
  });


  it('should update state when game is a tie', () => {
    const expectedState = {
      winner: null,
      end: true,
      currentPlayer: 'X',
    ...
    const result = game(xState, Actions.endGameWithNoWinner());
